package com.flutterbeaconv2.flutter_beacon_v2;
...
class FlutterBeaconV2Scanner {
  private static final String TAG = FlutterBeaconV2Scanner.class.getSimpleName();
  private final FlutterBeaconV2Plugin plugin;
  ...
  public FlutterBeaconV2Scanner(FlutterBeaconV2Plugin plugin, Activity activity) {
    this.plugin = plugin;
    ...
    if (o instanceof List) {
      ...
      for (Object object : list) {
        if (object instanceof Map) {
          Map map = (Map) object;
          Region region = FlutterBeaconv2Utils.regionFromMap(map);
          if (region != null) {
  ...
  private final RangeNotifier rangeNotifier = new RangeNotifier() {
    ...
    public void didRangeBeaconsInRegion(Collection<Beacon> collection, Region region) {
      if (eventSinkRanging != null) {
        ...
        map.put("region", FlutterBeaconv2Utils.regionToMap(region));
        map.put("beacons", FlutterBeaconv2Utils.beaconsToArray(new ArrayList<>(collection)));
  ...
  private final MonitorNotifier monitorNotifier = new MonitorNotifier() {
    ...
    public void didDetermineStateForRegion(int state, Region region) {
      if (eventSinkMonitoring != null) {
        ...
        map.put("state", FlutterBeaconv2Utils.parseState(state));
        map.put("region", FlutterBeaconv2Utils.regionToMap(region));
        handler.post(new Runnable(){
          @Override
          public void run() {
